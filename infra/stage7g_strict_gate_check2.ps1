...
function Invoke-Api {
  ...
  try {
    if ($JsonBody -ne '') {
      $obj = Invoke-RestMethod -Method $Method -Uri $Uri -Headers @{ Authorization = "Bearer $Token" } -ContentType 'application/json' -Body $JsonBody
    } else {
      $obj = Invoke-RestMethod -Method $Method -Uri $Uri -Headers @{ Authorization = "Bearer $Token" }
    }
    return @{ status=200; body=($obj | ConvertTo-Json -Depth 12) }
  } catch {
...
$workflowId=''
if ($g1.status -eq 200) { $workflowId = (ConvertFrom-Json $g1.body).workflow_id }
...
[PSCustomObject]@{ gate1=$g1; workflow_id=$workflowId; gate2=$g2; gate3=$g3; gate4_audit=$audit; gate5=$g5; gate6=$g6 } | ConvertTo-Json -Depth 12
