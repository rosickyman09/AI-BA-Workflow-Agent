...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("用法：")
        print("  python template_db.py upload <file.docx> [名稱]")
        print("  python template_db.py list")
        print("  python template_db.py get <template_id>")
        print("  python template_db.py delete <template_id>")
        sys.exit(1)
    ...
    if cmd == "upload" and len(sys.argv) >= 3:
        ...
        result = db.upload_template(sys.argv[2], name)
        print(f"\n模板 ID：{result['id']}")
        ...
        print(f"\n共 {len(templates)} 個模板：")
        for t in templates:
            print(f"  [{t['id']}] {t['name']} ({t['mode']}) - {t['created_at'][:10]}")

    elif cmd == "get" and len(sys.argv) >= 3:
        info = db.get_template(sys.argv[2])
        print(json.dumps(info["structure"], ensure_ascii=False, indent=2))

    elif cmd == "delete" and len(sys.argv) >= 3:
        db.delete_template(sys.argv[2])
        print("已刪除")
